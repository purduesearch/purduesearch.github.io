    ...
    <div className="clubpm-member-badge group relative inline-block">
      {member.avatarUrl ? (
        <img
          ...
          className={`${s.avatar} rounded-full ${s.ring} ring-[var(--clubpm-surface-100)] object-cover`}
        />
      ) : (
        <div
          className={`${s.avatar} rounded-full ${s.ring} ring-[var(--clubpm-surface-100)] flex items-center justify-center font-semibold ${s.text}`}
          style={{
            background: `linear-gradient(135deg, var(--clubpm-accent-primary), var(--clubpm-accent-pink))`,
            color: "white",
          }}
        >
          {initials}
