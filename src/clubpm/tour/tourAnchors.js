...
export const TOUR_ANCHORS = Object.freeze({
  ...
  "notifications.vault":    { label: "Vault notification settings", route: "/clubpm/notifications/preferences", note: "Delivery per Vault event plus the auto-watch checkbox" },
  ...
  "vault.repository":       { label: "Vault repository",     route: "/clubpm/projects/:id", note: "Files → Vault storage header" },
  "vault.tree":             { label: "Item list",            route: "/clubpm/projects/:id", note: "Files → Vault" },
  "vault.item":             { label: "Vault item row",       route: "/clubpm/projects/:id", note: "First row only" },
  "vault.item.modal":       { label: "Vault item detail",    route: "/clubpm/projects/:id", note: "Item detail panel, including checkout holder" },
  "vault.checkout":         { label: "Check-out button",     route: "/clubpm/projects/:id", note: "Inside the item modal" },
  "vault.release":          { label: "Release checkout",     route: "/clubpm/projects/:id", note: "Only shown while a checkout is held; a successful check-in releases it automatically" },
  "vault.upload":           { label: "Check in file",        route: "/clubpm/projects/:id", note: "Vault toolbar; creates an item" },
  "vault.item.upload":      { label: "New check-in",        route: "/clubpm/projects/:id", note: "Inside item detail; creates a version" },
  "vault.upload.form":      { label: "Check-in form",       route: "/clubpm/projects/:id", note: "File, required change description, transfer and processing status" },
  "vault.versions":         { label: "Version history",      route: "/clubpm/projects/:id", note: "Tab inside the item modal" },
  "vault.changes":          { label: "Version changes",      route: "/clubpm/projects/:id", note: "Inside the item Changes tab" },
  "vault.geometryDiff":     { label: "Geometry diff",        route: "/clubpm/projects/:id", note: "Inside vault.changes when both versions are STL/OBJ/glTF/GLB/STEP: units, tolerance, measurements, aligned views" },
  "vault.bom":              { label: "BOM view",             route: "/clubpm/projects/:id", note: "Tab inside the item modal" },
  "vault.drawingFor":       { label: "Drawing link",         route: "/clubpm/projects/:id", note: "BOM tab: mark this item as the drawing of another item" },
  "vault.tab.crs":          { label: "Change Requests tab",  route: "/clubpm/projects/:id", note: "Vault sub-nav pill" },
  "vault.tab.search":       { label: "Vault Search tab",     route: "/clubpm/projects/:id", note: "Vault sub-nav pill" },
  "vault.search":           { label: "Vault search panel",   route: "/clubpm/projects/:id", note: "Under vault.tab.search: query, filters, saved views, results" },
  "vault.watch":            { label: "Watch item",           route: "/clubpm/projects/:id", note: "Inside the item modal; the chevron picks events" },
  "cr.new":                 { label: "New change request",   route: "/clubpm/projects/:id", note: "Under vault.tab.crs" },
  "cr.form":                { label: "Change request form",  route: "/clubpm/projects/:id", note: "Title, Reason, optional task, item and version; submit here" },
  "cr.modal":               { label: "Change request detail", route: "/clubpm/projects/:id", note: "Request detail remains open after a decision" },
  "cr.reject":              { label: "Reject CR",            route: "/clubpm/projects/:id", note: "Admin-only rejection button in an OPEN CR; prompts for a reason" },
  ...
  "cr.review":              { label: "Approve CR",           route: "/clubpm/projects/:id", note: "Admin-only approval button inside an OPEN CR" },
  "cr.prReview":            { label: "PR and release review", route: "/clubpm/projects/:id", note: "Linked PR checks, reviewer sign-off, status, and timeline" },
  "cr.rules":               { label: "Reviewer rules",      route: "/clubpm/projects/:id", note: "Admin editor for subsystem and BOM parent sign-off rules" },
  "cr.readiness":           { label: "Build readiness",     route: "/clubpm/projects/:id", note: "Open CR only: blockers vs warnings, where-used, missing drawings, stale assemblies, affected tasks" },
  "cr.release":             { label: "Release package",     route: "/clubpm/projects/:id", note: "Approved CR only: manifest hash, pinned files, package download and verify" },
  "cr.drawingRules":        { label: "Drawing requirements", route: "/clubpm/projects/:id", note: "Admin editor for required drawings (warning or blocker)" },
