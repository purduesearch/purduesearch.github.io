...
- `src/pages/ClubPM/ProjectDetail.jsx` — Main PM view. Tabs: tasks, files (Drive/GitHub/Vault sub-tabs), chat, members, reports, and AI. Drag uses **`@dnd-kit/core`** (not `@hello-pangea/dnd`) with custom collision detection; member chips are draggable onto tasks/blockers. Optimistic updates use rollback; bulk moves go through `PATCH /api/tasks/bulk`.
