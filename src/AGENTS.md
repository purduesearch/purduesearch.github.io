...
- `src/pages/ClubPM/ProjectDetail.jsx` — Main PM view. The live taxonomy is the four `NAV_TABS`: **Tasks, Files, Chat, Insights**. Files has Drive / GitHub / Vault (a desktop pill row, a labelled **Source** selector on phones, both remembering `cpm.files.sub.<projectId>` in `sessionStorage`); Chat holds Messages and Members; Insights holds Charts, Activity, Press Kit and AI. The general Drive source links project documents. Enabled Vault repositories store new check-ins in GitHub/LFS; legacy versions remain provider-pinned. `?tab=members|reports|ai` are kept as redirects into those. Drag uses **`@dnd-kit/core`** (not `@hello-pangea/dnd`) with custom collision detection; member chips are draggable onto tasks/blockers. Optimistic updates use rollback; bulk moves go through `PATCH /api/tasks/bulk`.
