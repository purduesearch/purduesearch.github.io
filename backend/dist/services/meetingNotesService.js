...
function computeVelocityDelta(current, previous) {
...
// ── Main export ──────────────────────────────────────────────
export async function generateWeeklyMeetingTemplate(weekEndDate) {
    const now = new Date();
    ...
    // ── Per-project stats ────────────────────────────────────
    const projectSummaries = await Promise.all(activeProjects.map(async (project) => {
        const [tasksCreated, tasksCompleted, tasksBlocked, tasksOverdue, completedMilestones, recentLogs, milestoneLogs,] = await Promise.all([
        ...
        const keyChanges = recentLogs.map((log) => {
            ...
            const description = (typeof payload.taskTitle === "string" ? payload.taskTitle : null) ??
    ...
    // ── Club-wide stats ──────────────────────────────────────
    const totalTasksCompleted = projectSummaries.reduce((sum, p) => sum + p.tasksCompleted, 0);
    const totalNewTasks = projectSummaries.reduce((sum, p) => sum + p.tasksCreated, 0);
...
//# sourceMappingURL=meetingNotesService.js.map