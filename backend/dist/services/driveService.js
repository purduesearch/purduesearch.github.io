...
export async function fetchDriveFileAsText(fileId) {
    try {
        ...
        if (EXPORT_MAP[mimeType]) {
            const res = await drive.files.export({ fileId, mimeType: EXPORT_MAP[mimeType] }, { responseType: "text" });
            text = res.data;
        }
...
export async function fetchDriveFileAsBase64(fileId) {
    try {
        const auth = getDriveAuth();
        const drive = google.drive({ version: "v3", auth });
