...
import { riskAnalysisPrompt, sprintPlanPrompt, projectBriefPrompt, standupSynthesisPrompt, standupSentimentPrompt, inferDependenciesPrompt, capacityAnalysisPrompt, stakeholderEmailPrompt } from "../utils/aiPrompts.js";
...
export async function generateSprintPlan(projectId, capacityPoints = 40, sprintDays = 14) {
    const project = await prisma.project.findUnique({
        where: { id: projectId },
        include: { tasks: { include: { assignees: { select: { displayName: true } } } } },
    });
...
export async function generateProjectBrief(projectId) {
    ...
    if (!project)
        return null;
    return generateJson(projectBriefPrompt({ name: project.name, description: project.description, type: project.type, targetDate: project.targetDate?.toISOString().split("T")[0] ?? null }, project.tasks.map(t => ({ title: t.title, status: t.status, priority: t.priority })), milestones.map((m) => ({ name: m.title, targetDate: m.dueDate?.toISOString().split("T")[0] ?? null, status: m.status })), project.updates.map(u => u.content.slice(0, 100))), `brief:${projectId}`);
}
export async function synthesizeStandups(responses, projectName) {
    if (responses.length === 0)
        return null;
    return generateJson(standupSynthesisPrompt(responses, projectName));
}
export async function analyzeStandupSentiment(responses) {
    if (responses.length === 0)
        return null;
    return generateJson(standupSentimentPrompt(responses));
}
export async function inferTaskDependencies(projectId) {
...
export async function analyzeTeamCapacity(projectId, sprintDays = 14) {
    const project = await prisma.project.findUnique({
        where: { id: projectId },
        include: {
            tasks: {
...
export async function generateStakeholderEmail(projectId) {
    ...
    return generateJson(stakeholderEmailPrompt({ name: project.name, targetDate: project.targetDate?.toISOString().split("T")[0] ?? null }, completedThisWeek, inProgress, blockers, healthScore));
}
