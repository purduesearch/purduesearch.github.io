import { buildWeeklyDigest, buildProjectStatusCard, buildTaskReminderCard, buildWeekAheadCard, buildBatchedReminderDigest, buildEscalationCard, buildTaskCard, buildStandupDmPrompt, buildStaleTasksCard, buildMilestoneCelebrationCard, buildMilestoneAlertCard, } from "../utils/blockKit.js";
import { prisma } from "../db/prisma.js";
import { getOverdueTasks, getTasksDueToday, getTasksDueThisWeek, } from "./taskService.js";
// ── Notification Target CRUD ─────────────────────────────────
export async function addNotificationTarget(projectId, type, slackChannelId, eventTypes = []) {
...
export async function removeNotificationTarget(id) {
    return prisma.projectNotificationTarget.delete({ where: { id } });
}
export async function getNotificationTargetsForProject(projectId) {
...
export async function sendWeeklyDigest(app, member) {
    if (!member.notificationPrefs.includes("weekly_digest"))
        return;
    const tasks = (await getTasksDueThisWeek(member.id));
    if (tasks.length === 0)
        return;
...
export async function sendDueDateReminder(app, task) {
...
export async function postProjectHealthSummary(app, project) {
    const channels = await getChannelsForProject(project.id);
    if (channels.length === 0)
        return;
    const blocks = buildProjectStatusCard(project, project.tasks);
    await Promise.all(channels.map(channel => app.client.chat.postMessage({
        channel,
        text: `📊 Weekly health summary for ${project.name}`,
        blocks,
    })));
}
export async function postWeekAheadSummary(app, project) {
    const channels = await getChannelsForProject(project.id);
    if (channels.length === 0)
        return;
    const now = new Date();
...
export async function sendAllDueDateReminders(app) {
    const overdueTasks = (await getOverdueTasks());
    const dueTodayTasks = (await getTasksDueToday());
    ...
    for (const { member, overdue, dueToday } of byMember.values()) {
        try {
            ...
            if (!fullMember?.notificationPrefs.includes("daily_reminders"))
                continue;
...
export async function sendAllWeeklyDigests(app) {
    ...
    for (const member of members) {
        try {
            ...
            console.error(`Failed to send digest to ${member.displayName}:`, error);
        }
    }
}
export async function postAllProjectHealthSummaries(app) {
...
export async function postAllWeekAheadSummaries(app) {
    const targetProjectIds = await prisma.projectNotificationTarget.findMany({
        select: { projectId: true },
        distinct: ["projectId"],
    });
...
export async function postEscalationNotice(app, task) {
...
export async function sendAllEscalations(app) {
...
export async function postTaskAnnouncement(app, task, channelId) {
    try {
...
export async function postTaskThreadUpdate(app, task, updateText) {
    const channels = await getChannelsForProject(task.projectId);
    if (channels.length === 0)
        return;
...
// ── Standup Prompts ───────────────────────────────────────────
export async function sendStandupPrompts(app) {
    const members = await prisma.member.findMany({
...
// ── Milestone Alerts ─────────────────────────────────────────
export async function sendMilestoneAlerts(app, changed) {
...
export async function sendCombinedMondayDigest(app) {
    const members = await prisma.member.findMany({
        where: {
            OR: [
    ...
    for (const member of members) {
        try {
            ...
            const digestBlocks = member.notificationPrefs.includes("weekly_digest") && tasks.length > 0
...
// ── Stale Task Warnings ───────────────────────────────────────
export async function sendAllStaleTaskWarnings(app) {
