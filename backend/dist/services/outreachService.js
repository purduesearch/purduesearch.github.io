...
export async function reviewSubmission(id, reviewerId, status, note) {
    return prisma.outreachSubmission.update({
        where: { id },
        data: {
            status,
...
export async function getOutreachDigest() {
    ...
    const [recentMilestonesRaw, upcomingEventsRaw, pendingCount, approvedCount] = await Promise.all([
...
export async function getContentCalendar(from, to) {
    return prisma.outreachSubmission.findMany({
        where: {
