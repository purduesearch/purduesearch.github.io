...
function buildRecurringCopies(base, pattern) {
...
export async function createEvent(data) {
    ...
    if (data.isRecurring && data.recurrencePattern) {
        const offsets = buildRecurringCopies({ title: data.title, type: data.type ?? "MEETING", startTime: data.startTime, endTime: data.endTime }, data.recurrencePattern);
        if (offsets.length > 0) {
            await Promise.all(offsets.map(startTime => {
...
export async function getUpcomingEvents(days) {
    const now = new Date();
