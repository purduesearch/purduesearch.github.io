...
export function registerActions(app) {
    ...
    app.action("mark_done", async ({ action, ack, respond, body, client }) => {
        ...
        try {
            ...
            if (task.isRecurring) {
                ...
                if (fullTask)
                    await spawnNextOccurrence(fullTask).catch(console.error);
            }
            ...
            if (task.slackMsgTs) {
                const msgChannel = "channel" in body && body.channel
                    ? body.channel.id
                    : null;
                if (msgChannel) {
                    try {
    ...
    app.action("create_task_from_message", async ({ action, ack, respond, body, client }) => {
        await ack();
        try {
            if (!("value" in action) || !action.value)
                return;
            ...
            const initialTitle = action.value
                ...
                .trim()
                .slice(0, 200);
            const channelId = "channel" in body && body.channel
                ? body.channel.id
                : "";
    ...
    app.action("open_task_modal_from_reaction", async ({ action, ack, body, client, respond }) => {
        await ack();
        try {
            if (!("value" in action) || !action.value)
                return;
    ...
    app.action("ai_create_task", async ({ action, ack, body, client }) => {
        ...
        try {
            ...
            const isAdminUser = await isAdminBySlackId(body.user.id);
            await openNewTaskModal(client, body.trigger_id, channelId, cached?.title, cached?.description, cached?.dueDate, cached?.suggestedAssigneeSlackIds, cached?.parentTaskId, isAdminUser);
        }
        catch (error) {
    ...
    app.action("ai_enrich_task", async ({ action, ack, respond }) => {
        ...
        try {
            ...
            const task = await getTask(taskId);
            if (!task) {
                await respond({ text: "❌ Task not found.", response_type: "ephemeral" });
                return;
            }
            const { generateJson } = await import("../services/geminiService.js");
            const { enrichTaskPrompt } = await import("../utils/aiPrompts.js");
            const projectType = task.project?.type ?? "engineering";
            const enriched = await generateJson(enrichTaskPrompt(task.title, task.description ?? "", projectType));
            if (!enriched) {
                await respond({ text: "❌ AI enrichment failed.", response_type: "ephemeral" });
                return;
            }
    ...
    app.action("ai_suggest_deadline", async ({ action, ack, respond }) => {
        ...
        try {
            ...
            const project = await prisma.project.findUnique({ where: { id: task.projectId }, select: { targetDate: true } });
            const today = new Date().toISOString().split("T")[0];
            const result = await generateJson(deadlineSuggestionPrompt(task.title, task.description ?? "", task.storyPoints ?? null, 10, project?.targetDate?.toISOString().split("T")[0] ?? null, today));
            if (!result)
                return;
            await respond({
                response_type: "ephemeral",
    ...
    app.view("event_create_submit", async ({ ack, view, body }) => {
        ...
        try {
            ...
            const type = (v.event_type?.value?.selected_option?.value ?? "MEETING");
    ...
    app.view("outreach_submit_modal", async ({ ack, view, body }) => {
        ...
        try {
            ...
            const type = (v.submission_type?.value?.selected_option?.value ?? "SOCIAL_POST");
            const content = v.submission_content?.value?.value ?? undefined;
            const platform = (v.submission_platform?.value?.selected_options ?? []).map((o) => o.value);
            const status = (v.submission_status?.value?.selected_option?.value ?? "SUBMITTED");
            if (!title)
                return;
            const member = await resolveSlackMember(body.user.id);
