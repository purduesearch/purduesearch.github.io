...
membersRouter.patch("/me/notification-preferences", async (req, res) => {
    try {
        if (!req.session.memberId) {
            res.status(401).json({ error: "Not authenticated" });
            return;
        }
        const { notificationPrefs, notificationChannels, quietHoursStart, quietHoursEnd, mutedProjectIds, } = req.body;
        ...
        if (mutedProjectIds !== undefined)
            updateData.mutedProjectIds = mutedProjectIds;
        const member = await prisma.member.update({
            where: { id: req.session.memberId },
            data: updateData,
            select: {
                    ...
                    include: {
                        project: {
                            include: {
                                _count: { select: { tasks: true } },
                                tasks: {
                                    where: { assignees: { some: { id: req.params.id } } },
