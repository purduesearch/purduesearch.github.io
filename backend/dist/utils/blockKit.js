...
// ── Task Card ────────────────────────────────────────────────
export function buildTaskCard(task, project, options) {
    ...
    if (task.recurringInterval) {
        const intervals = { DAILY: "daily", WEEKLY: "weekly", BIWEEKLY: "bi-weekly", MONTHLY: "monthly" };
        infoLines.push(`🔄 Repeats ${intervals[task.recurringInterval] ?? task.recurringInterval.toLowerCase()}`);
    }
...
// ── Project Status Card ──────────────────────────────────────
export function buildProjectStatusCard(project, tasks) {
...
// ── Weekly Digest ────────────────────────────────────────────
export function buildWeeklyDigest(member, tasks) {
    const taskLines = tasks
        .map((t) => `${priorityEmoji(t.priority)} *${t.title}* — ${t.project.name} • Due: ${formatDate(t.dueDate)}`)
        .join("\n");
    return [
        {
            type: "header",
...
// ── Task Reminder ────────────────────────────────────────────
export function buildTaskReminderCard(task) {
...
// ── Week Ahead Card ──────────────────────────────────────────
export function buildWeekAheadCard(project, tasks) {
    const taskLines = tasks
        .map((t) => `${statusEmoji(t.status)} *${t.title}* — ${t.assignees && t.assignees.length > 0 ? t.assignees.map(a => `<@${a.slackId}>`).join(", ") : "_Unassigned_"} • Due: ${formatDate(t.dueDate)}`)
        .join("\n");
    return [
        {
            type: "header",
...
// ── Standup Message ──────────────────────────────────────────
export function buildStandupMessage(authorSlackId, responses) {
    return [
        {
            type: "header",
            text: {
...
export function buildTodoPrompt(messageText) {
    return [
        {
            type: "section",
            text: {
...
export function buildBatchedReminderDigest(member, overdueTasks, dueTodayTasks) {
    const blocks = [];
    blocks.push({
        type: "header",
...
// ── Escalation Card ──────────────────────────────────────────
export function buildEscalationCard(task) {
...
// ── App Home ─────────────────────────────────────────────────
export function buildAppHome(member, tasks, frontendUrl) {
    const now = new Date();
    const endOfWeek = new Date(now);
    endOfWeek.setDate(now.getDate() + 7);
    const overdueTasks = tasks.filter((t) => t.dueDate && t.dueDate < now && t.status !== "DONE");
    const overdueIds = new Set(overdueTasks.map((t) => t.id));
    const inProgressTasks = tasks.filter((t) => t.status === "IN_PROGRESS" && !overdueIds.has(t.id));
...
export function buildAiTaskSuggestion(parsed, channelId, cacheKey, suggestedAssigneeSlackIds = []) {
    const lines = [`📋 *Suggested task:* ${parsed.title}`];
    if (parsed.description)
        lines.push(`_${parsed.description}_`);
    if (parsed.priority)
        lines.push(`Priority: ${priorityEmoji(parsed.priority)} ${parsed.priority}`);
    if (parsed.dueDate)
        lines.push(`Due: ${parsed.dueDate}`);
...
// ── Standup DM Prompt ────────────────────────────────────────
export function buildStandupDmPrompt(displayName, projectChannels) {
    const blocks = [
        {
            type: "section",
            text: {
    ...
    if (projectChannels.length === 0) {
        ...
        blocks.push({
            ...
            elements: projectChannels.slice(0, 3).map((pc) => ({
                type: "button",
                text: { type: "plain_text", text: `📝 ${pc.name}`.slice(0, 25) },
...
export function buildProjectReport(project, tasks, milestones, statusCounts, overdueCount) {
    ...
    if (overdueCount > 0) {
        const lines = overdueTasks.map(t => {
            const daysOver = Math.floor((now.getTime() - t.dueDate.getTime()) / 86_400_000);
    ...
    if (sortedWorkload.length > 0) {
        const lines = sortedWorkload.map(([id, w]) => `• ${id === "__unassigned__" ? "_Unassigned_" : `<@${w.slackId}>`}: ${w.count} task${w.count !== 1 ? "s" : ""}`);
        blocks.push({
            type: "section",
...
// ── Project Health ────────────────────────────────────────────
export function buildProjectHealth(project, tasks, milestones, statusCounts, overdueCount) {
    const total = Object.values(statusCounts).reduce((a, b) => a + b, 0);
    const done = statusCounts.DONE ?? 0;
    const blocked = statusCounts.BLOCKED ?? 0;
    const now = new Date();
    ...
    const unassignedCount = tasks.filter(t => !t.parentTaskId && t.status !== "DONE" && t.assignees.length === 0).length;
    ...
    const overdueMilestones = milestones.filter(m => m.dueDate && m.dueDate < now && m.taskCounts.done < m.taskCounts.total).length;
    ...
    if (done > 0)
        positives.push(`✅ ${done} task${done > 1 ? "s" : ""} completed`);
    if (total > 0)
        positives.push(`📊 ${Math.round((done / total) * 100)}% overall completion`);
    if (project.targetDate)
        positives.push(`🎯 Target: ${formatDate(project.targetDate)}`);
...
// ── Milestone View ────────────────────────────────────────────
export function buildMilestoneView(project, milestones) {
    const now = new Date();
    const blocks = [];
    blocks.push({
        type: "header",
...
export function buildMilestoneCelebrationCard(title, projectName) {
    return [
...
// ── AI Risk Report ────────────────────────────────────────────
export function buildRiskReport(project, risks) {
    ...
    if (risks.risks.length > 0) {
        ...
        const lines = risks.risks.slice(0, 5).map(r => `${severityEmoji[r.severity] ?? "⚪"} *${r.category}*: ${r.description}`);
        blocks.push({
            type: "section",
...
// ── AI Capacity Report ────────────────────────────────────────
export function buildCapacityReport(project, cap) {
...
// ── Drive Task Preview ────────────────────────────────────────
export function buildDriveTaskPreview(tasks, channelId) {
    ...
    for (const task of tasks.slice(0, 10)) {
        ...
        if (task.suggestedAssigneeName)
            meta.push(`👤 ${task.suggestedAssigneeName}`);
        blocks.push({
            type: "section",
            text: {
                type: "mrkdwn",
...
export function buildDependencySuggestionsBlocks(dependencies, _projectId) {
    const blocks = [
        {
            type: "header",
...
// ── Standup Digest ────────────────────────────────────────────
export function buildStandupDigestBlocks(synthesis, projectName) {
    const momentumEmoji = { STALLED: "🔴", SLOW: "🟡", STEADY: "🟢", STRONG: "💪" };
...
//# sourceMappingURL=blockKit.js.map