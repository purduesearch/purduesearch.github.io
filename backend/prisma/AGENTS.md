...
Key models include `Member`, `Project`, `ProjectMember`, `Task`, `TaskComment`, `TaskDependency`, `Blocker`, `TaskBlocker`, `Milestone`, `TimeLog`, `Activity`, `ActivityLog`, `GitHubLink`, `ProjectRepo`, `Event`, `OutreachSubmission`, `BlogPost`, `Course`, `CourseModule`, `CourseSection`, `CourseWorkSubmission`, `Training`, `TrainingCertificate`, `XpEvent`, `DoubloonEvent`, `Challenge`, `MemberChallenge`, `MemberAchievement`, `Cosmetic`, `MemberCosmetic`, `MemberInventory`, `SlackMessage`, `SlackMessageFile`, `SlackConversationMember`, `Workspace`, `WorkspaceProject`, `WorkspaceRequirement`, `LabShift`, `LabShiftSkip`, `VaultDrawingRequirement`, `VaultRelease`, and the Phase 8 `VaultSubscription`, `VaultNotificationEvent`, `VaultNotificationDelivery`, `VaultSearchDoc`, and `VaultSavedView`, and the Phase 9 `VaultGeometryDiff` (cache row unique per `projectId` + `cacheKey`; `resultJson` TEXT, derived and prunable). `VaultSearchDoc.searchVector` is `Unsupported("tsvector")` with a GIN index; only the raw upsert in `vaultSearchService.ts` writes it, and after a deploy that adds or changes index documents run `npm run vault:search-rebuild`. `VaultNotificationDelivery`'s `@@unique([eventId, recipientId, channel])` is the deduplication guarantee — never drop it. `VaultRelease.manifestJson` is immutable TEXT (not JSONB, which reorders keys) hashed into `manifestSha256`; only the `package*`/`leaseUntil` columns change after the approval transaction inserts it. `VaultItem.drawingForId` marks a drawing item. `LabShift` dates are `@db.Date` local dates and minutes are local; `Event.workspaceId` links an event to a lab space (SetNull). Search the schema for the authoritative fields and relations before writing queries; there is no `MilestoneTask` or `InventoryItem` model in the current schema.
