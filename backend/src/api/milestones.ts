...
milestonesRouter.get("/project/:projectId", async (req: Request, res: Response) => {
  try {
    ...
    const enriched = milestones.map((m) => {
      const tasks = (m as any).tasks as { status: string }[];
...
milestonesRouter.post("/", async (req: Request, res: Response) => {
  try {
    const { title, projectId, dueDate, description, ownerId } = req.body as {
      title: string;
      projectId: string;
      dueDate?: string;
      description?: string;
...
milestonesRouter.patch("/:id", async (req: Request, res: Response) => {
  try {
    ...
    const { title, dueDate, description, ownerId, status, milestoneTaskIds } = req.body as {
      title?: string;
      dueDate?: string | null;
      description?: string;
      ownerId?: string | null;
