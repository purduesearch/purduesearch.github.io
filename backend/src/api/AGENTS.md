...
- `projects.ts` — Project CRUD; also mounts `tagsRouter`. `GET /:id/time-insights?from=&to=` (default last 12 weeks) returns manual vs lab-check-in minutes per week, member and task via `services/timeInsightsService.ts`; `unallocatedLabMinutes` counts only visits in spaces linked to exactly this one project.
...
- `vault.ts` + `vaultGithub.ts` + `changeRequests.ts` — Constellation Vault CAD/PDM: provider-pinned versions, GitHub repository/jobs, legacy Drive reads/writes during migration, checkouts, BOM, CRs. Mounted at bare `/api` (like `blockers.ts` and `streak.ts`). Change-request review routes link/sync a PR, expose the review gate, and record member sign-off; project reviewer-rule editing is admin-only.
- `vaultSearch.ts` — Phase 8, bare `/api`: `GET /vault/search` (q, projectId, kinds, released, checkedOut, crStatus, fileExts, authorId=me, watching, since, until, sort, limit, offset — scoped to the caller's projects, 403 for a named project they cannot open), saved views `GET|POST /vault/saved-views`, `PATCH|DELETE /vault/saved-views/:id` (owner only; queries normalized by `savedViewQuery`), `GET|PUT|DELETE /vault/items/:id/subscription` (DELETE stores all-false), `GET /vault/subscriptions`, and admin `POST /projects/:projectId/vault/search/rebuild`. Links in results and notifications use `vaultLink()`: `/clubpm/projects/:id?tab=files&sub=vault&vaultItem=&vaultVersion=` or `&vaultCr=`.
- `vaultGeometry.ts` — Phase 9, bare `/api`, every route `requireAuth`: `GET /vault/geometry/capabilities`, `POST /vault/geometry-diffs` ({ beforeVersionId, afterVersionId, units?, tolerance? } → cached row or 202 queued; 403 before revealing anything about a project you cannot open, 422 unsupported format, 409 legacy version without SHA-256), `GET /vault/geometry-diffs/:id`, `GET /vault/geometry-diffs/:id/mesh/:side` (a STEP side's tessellation as binary STL). Reads re-check the row's project.
- `vaultReleases.ts` — `/api/vault-releases/:id` (release + parsed manifest + stored readiness), `/manifest` (exact stored bytes), `/package-url` (signed link), `GET /package` (tar built only from the manifest), `POST /package` (retry/build), `POST /verify` (rebuild + compare hash). **Mounted above every bare `/api` router** because the package download accepts a signed `?sig=` link; `appMountOrder.test.ts` guards it. Project/CR-scoped Phase 7 routes live in `changeRequests.ts`: `GET /change-requests/:id/readiness`, `GET /change-requests/:id/release`, `GET|PUT /projects/:projectId/vault/drawing-requirements` (PUT admin), `PUT /vault/items/:id/drawing-for`, `GET /projects/:projectId/vault/releases`.
