 ...
 */
export function initDmBatcher(app: App): void {
  slackApp = app;
...
export function queueDm(slackId: string, message: string): void {
  ...
  if (entry) {
    ...
    // Create new entry
    entry = { messages: [], timer: null as any };
 ...
 */
export async function flushDm(slackId: string): Promise<void> {
  if (!slackApp) {
    console.warn(
      "⚠️  DM Batcher not initialized. Call initDmBatcher() at startup."
    );
  ...
  try {
    ...
    console.log(
      `✉️  Sent batched DM to ${slackId} (${messages.length} message${messages.length === 1 ? "" : "s"})`
    );
  } catch (error) {
    console.error(
      `❌ Failed to send batched DM to ${slackId}:`,
