...
export async function getProjectBurndown(projectId: string) {
  ...
  const now = new Date();
  const points: { date: string; remaining: number; completed: number; total: number }[] = [];
  ...
  while (cursor <= now) {
    ...
    const completed = tasks.filter(
      (t) => t.status === "DONE" && t.updatedAt <= dayEnd
    ).length;
...
export async function getOverdueCount(projectId: string) {
  const now = new Date();
